...
import android.app.*;import android.os.*;import android.database.Cursor;import android.database.sqlite.*;import android.speech.tts.TextToSpeech;import android.graphics.*;import android.graphics.drawable.GradientDrawable;import android.content.*;import android.net.Uri;import android.text.*;import android.view.*;import android.view.inputmethod.EditorInfo;import android.widget.*;
...
public class MainActivity extends Activity {
 ...
 final String BACKEND="http://192.168.100.4:8000";
...
status=tv("Connecting…",10,MUTED);head.addView(status);root.addView(head);
  ...
  nav=new LinearLayout(this);nav.setPadding(dp(6),dp(5),dp(6),dp(8));nav.setBackgroundColor(Color.rgb(11,15,21));String[] n={"HOME","SCREENER","PULSE","NEWS","INTEL","PORTFOLIO","MORE"};for(String x:n){Button b=btn(x);b.setOnClickListener(v->{screen=x;render();});nav.addView(b,new LinearLayout.LayoutParams(0,dp(48),1));}root.addView(nav);
 }
 ...
 void refresh(){status.setText("↻ Refreshing");pool.submit(()->{
   ArrayList<Stock> d; String src;
   try{d=fetchBackend();src="backend";}
   catch(Exception primaryErr){
     // Backend unreachable (wrong LAN, server down, etc.) — fall back to
     // scraping PSX's public market-watch page directly rather than just
     // failing. fetchDirect() already existed but was never wired in here.
     try{d=fetchDirect();src="direct (backend unreachable)";}
     catch(Exception directErr){h.post(()->status.setText("Data error • tap refresh to retry"));return;}
   }
   final ArrayList<Stock> data=d; final String source=src;
   h.post(()->{all=data;status.setText("● "+marketClock()+" • 5M DELAY • "+source+" • "+new SimpleDateFormat("HH:mm:ss",Locale.US).format(new Date()));render();});
 });}
    ArrayList<Stock> fetchBackend() throws Exception {

        URL u = new URL(BACKEND+"/market");
        ...
        for(int i=0;i<arr.length();i++){
            ...
            z.s = x.getString("symbol");
            z.sector = x.optString("sector","");
            z.listed = x.optString("listed","");

            z.ldcp = x.optDouble("ldcp",0);
            z.o = x.optDouble("open",0);
            z.hi = x.optDouble("high",0);
            z.lo = x.optDouble("low",0);

            z.p = x.optDouble("price",0);
            z.ch = x.optDouble("change",0);
            z.pct = x.optDouble("pct",0);
            z.vol = x.optDouble("volume",0);

            z.score = x.optDouble("score",0);
            z.setup = x.optString("setup","");

            z.sh = x.optBoolean("shariah",false);
 ...
 void render(){content.removeAllViews();if(screen.equals("HOME"))home();else if(screen.equals("SCREENER"))scanner();else if(screen.equals("PULSE"))markets();else if(screen.equals("NEWS"))newsFeed();else if(screen.equals("INTEL"))intelligence();else if(screen.equals("PORTFOLIO"))portfolioDashboard();else more();}
 ...
 void home(){hero("Command Center","Full PSX intelligence • tap any security to drill down");if(all.isEmpty()){content.addView(tv("  Loading market…",14,MUTED));return;}int adv=0,dec=0;double vv=0;for(Stock s:all){if(s.pct>0)adv++;if(s.pct<0)dec++;vv+=s.vol;}LinearLayout pulse=card();pulse.addView(tv("MARKET BREADTH",11,MUTED));double breadth=100.0*adv/Math.max(1,adv+dec);pulse.addView(bold(String.format(Locale.US,"%.0f%%  %s",breadth,breadth>=55?"RISK-ON":breadth<45?"DEFENSIVE":"MIXED"),24,breadth>=55?GREEN:breadth<45?RED:GOLD));pulse.addView(tv(adv+" advancing  •  "+dec+" declining  •  "+all.size()+" securities",12,MUTED));content.addView(pulse);
     ...
     intel.addView(tv(
             "Advancers vs decliners, liquidity and price behaviour are combined into the current market state. Always confirm with risk management.",
             12,
             MUTED
     ));
 ...
 void portfolioDashboard(){hero("Portfolio","Local positions, unrealized P&L and risk framework");
  LinearLayout summary=card();summary.addView(bold("Your positions",17,TEXT));
  if(portfolioDB==null)portfolioDB=new PortfolioDB(this);
  double totalPnl=0;int rows=0;
  for(Stock s:all){double p=portfolioDB.pnl(s.s,s.p);if(p!=0){totalPnl+=p;rows++;}}
  if(rows==0){summary.addView(tv("No stored positions yet. Positions you log will show unrealized P&L here based on live prices.",12,MUTED));}
  else{summary.addView(bold(String.format(Locale.US,"%+.2f",totalPnl),24,totalPnl>=0?GREEN:RED));summary.addView(tv(rows+" symbol(s) with tracked positions",12,MUTED));}
 ...
 void more(){hero("More","Research tools • PSX sources • watchlist • data status");String[] items={"TRADER DASHBOARD","CANDLE ENGINE","WATCHLIST ALERTS","SIGNAL HISTORY","PORTFOLIO & RISK","NEWS & DISCLOSURES","WATCHLIST","OFFICIAL ANNOUNCEMENTS","COMPANY FUNDAMENTALS","PAST PICKS / TRACK RECORD","METHODOLOGY & GLOSSARY"};for(String z:items){LinearLayout c=card();c.addView(bold(z,16,TEXT));String sub=z.equals("NEWS & DISCLOSURES")?"Official PSX disclosure hub + news intelligence":z.equals("WATCHLIST")?"Saved securities on this device":z.equals("OFFICIAL ANNOUNCEMENTS")?"Open the current PSX company-announcement portal":z.equals("COMPANY FUNDAMENTALS")?"Open a stock, then use Fundamentals for its official PSX company page":z.equals("PORTFOLIO & RISK")?"Local risk framework and position-sizing guide":z.equals("PAST PICKS / TRACK RECORD")?"Signal-accountability framework; no invented historical wins":"How V3.3 interprets Wyckoff, structure, candles and scores";c.addView(tv(sub,11,MUTED));c.setOnClickListener(v->{if(z.equals("TRADER DASHBOARD")){content.removeAllViews();traderDashboard();}else if(z.equals("CANDLE ENGINE")){content.removeAllViews();candleEngine();}else if(z.equals("WATCHLIST ALERTS")){content.removeAllViews();watchAlerts();}else if(z.equals("SIGNAL HISTORY")){content.removeAllViews();signalHistory();}else if(z.equals("NEWS & DISCLOSURES")){content.removeAllViews();newsFeedLegacy();}else if(z.equals("WATCHLIST")){content.removeAllViews();watchlist();}else if(z.equals("OFFICIAL ANNOUNCEMENTS"))openUrl("https://dps.psx.com.pk/announcements/companies");else if(z.equals("COMPANY FUNDAMENTALS"))toast("Open any stock card → Fundamentals");else if(z.equals("PORTFOLIO & RISK"))riskGuide();else if(z.equals("PAST PICKS / TRACK RECORD"))trackRecord();else methodology();});content.addView(c);}}
 void newsFeed(){
  hero("News & Impact","Headlines from credible desks • what each stock actually did");
  LinearLayout status=card();TextView st=tv("Loading news…",12,MUTED);status.addView(st);content.addView(status);
  final LinearLayout host=new LinearLayout(this);host.setOrientation(LinearLayout.VERTICAL);content.addView(host);
  pool.submit(()->{try{
    String json=getJson("/news-feed?max_age_minutes=30");
    JSONObject o=new JSONObject(json);
    h.post(()->{
      host.removeAllViews();
      Double age=o.isNull("age_minutes")?null:o.optDouble("age_minutes");
      st.setText("Headlines "+(age==null?"not fetched yet":String.format(Locale.US,"%.0f min old",age))
        +(o.optBoolean("refreshing_now",false)?" • refreshing…":"")
        +"\nQuotes are live (5-min delayed). News re-fetches only when stale.");
      if(!"ok".equals(o.optString("status",""))){
        LinearLayout c=card();c.addView(bold("No news verdicts yet",16,TEXT));
        c.addView(tv("The feed has not been built or is stale. It refreshes in the background — reopen this tab in a minute.",12,MUTED));
        host.addView(c);}

      JSONArray stocks=o.optJSONArray("stocks");
      if(stocks!=null&&stocks.length()>0){
        TextView h1=bold("  STOCKS IN THE NEWS",15,TEXT);h1.setPadding(dp(14),dp(10),0,dp(2));host.addView(h1);
        for(int i=0;i<stocks.length();i++){
          JSONObject z=stocks.optJSONObject(i);if(z==null)continue;
          final String symbol=z.optString("symbol","");
          LinearLayout c=card();
          LinearLayout top=new LinearLayout(this);top.setGravity(Gravity.CENTER_VERTICAL);
          top.addView(bold(symbol,18,TEXT),new LinearLayout.LayoutParams(0,-2,1));
          String dir=z.optString("direction","neutral");
          int dc=dir.equals("positive")?GREEN:dir.equals("negative")?RED:GOLD;
          top.addView(bold(dir.toUpperCase()+"  "+String.format(Locale.US,"%.0f",z.optDouble("news_score",50)),13,dc));
          c.addView(top);
          if(!z.isNull("observed_move_pct")){
            double mv=z.optDouble("observed_move_pct",0);
            c.addView(bold("Actual move today: "+String.format(Locale.US,"%+.2f%%",mv),14,mv>=0?GREEN:RED));
          }
          String agree=z.optString("agreement","");
          if(agree.length()>0&&!agree.equals("null"))c.addView(tv(agree,11,MUTED));
          c.addView(tv("News confidence: "+z.optString("confidence","—")
            +"   •   materiality: "+z.optString("materiality","—"),11,MUTED));
          JSONArray hl=z.optJSONArray("headlines");
          if(hl!=null)for(int k=0;k<hl.length();k++)c.addView(tv("• "+hl.optString(k),12,TEXT));
          JSONArray src=z.optJSONArray("sources");
          if(src!=null&&src.length()>0){
            final String url=src.optString(0);
            Button b=btn("Open source");b.setOnClickListener(v->openUrl(url));
            c.addView(b,new LinearLayout.LayoutParams(-1,dp(42)));
          }
          c.setOnClickListener(v->{for(Stock s2:all)if(s2.s.equals(symbol)){detail(s2);return;}toast("Quote not loaded for "+symbol);});
          host.addView(c);
        }
      }

      JSONArray macro=o.optJSONArray("macro_headlines");
      if(macro!=null&&macro.length()>0){
        TextView h2=bold("  MARKET & MACRO",15,TEXT);h2.setPadding(dp(14),dp(14),0,dp(2));host.addView(h2);
        for(int i=0;i<macro.length();i++){
          JSONObject m=macro.optJSONObject(i);if(m==null)continue;
          LinearLayout c=card();
          c.addView(bold(m.optString("title",""),14,TEXT));
          c.addView(tv(m.optString("publisher",""),11,MUTED));
          JSONArray th=m.optJSONArray("themes");
          if(th!=null&&th.length()>0){
            StringBuilder t=new StringBuilder();
            for(int k=0;k<th.length();k++){
              JSONObject x=th.optJSONObject(k);if(x==null)continue;
              t.append("Theme: ").append(x.optString("theme","")).append("\n");
              JSONArray se=x.optJSONArray("sectors_conventionally_sensitive");
              if(se!=null)for(int j=0;j<se.length();j++)t.append("  • ").append(se.optString(j)).append("\n");
            }
            c.addView(tv(t.toString().trim(),11,GOLD));
            c.addView(tv("Conventional analyst relationship — not measured against PSX history. Guidance for where to look, not evidence.",10,MUTED));
          }
          final String url=m.optString("url","");
          if(url.length()>0){Button b=btn("Read");b.setOnClickListener(v->openUrl(url));c.addView(b,new LinearLayout.LayoutParams(-1,dp(42)));}
          host.addView(c);
        }
      }

      LinearLayout foot=card();
      foot.addView(tv(o.optString("impact_note",""),10,MUTED));
      host.addView(foot);
    });
  }catch(Exception e){h.post(()->st.setText("News unavailable — backend not reachable. No headlines are generated locally."));}});
 }
 void newsFeedLegacy(){hero("News & Disclosures","Real source first • official PSX announcements are never synthesized");
...
void riskGuide(){content.removeAllViews();hero("Portfolio & Risk","Risk first • signals are not position sizes");LinearLayout c=card();c.addView(bold("V3.3 risk framework",17,TEXT));c.addView(tv("• Define invalidation before entry\n• Prefer adequate resistance headroom\n• Compare reward to risk\n• Avoid concentration in one sector/theme\n• Reduce size when volatility expands\n• Never convert a high score into certainty\n\nThe original V1 risk-manager modules are retained in the backend source for server deployment and validation.",13,TEXT));content.addView(c);}
 ...
 void detail(Stock s){content.removeAllViews();nav.setVisibility(View.GONE);LinearLayout head=card();Button back=btn("← Back");back.setOnClickListener(v->{nav.setVisibility(View.VISIBLE);render();});head.addView(back,new LinearLayout.LayoutParams(dp(90),dp(42)));LinearLayout r=new LinearLayout(this);r.setGravity(Gravity.CENTER_VERTICAL);LinearLayout names=new LinearLayout(this);names.setOrientation(LinearLayout.VERTICAL);names.addView(bold(s.s,30,TEXT));names.addView(tv(s.sector,12,MUTED));r.addView(names,new LinearLayout.LayoutParams(0,-2,1));TextView px=bold(String.format(Locale.US,"%.2f\n%+.2f%%",s.p,s.pct),19,s.pct>=0?GREEN:RED);px.setGravity(Gravity.RIGHT);r.addView(px);head.addView(r);content.addView(head);
  ...
  LinearLayout chart=card();chart.addView(bold("Interactive price history",16,TEXT));TextView cs=tv("Loading PSX EOD history…",11,MUTED);chart.addView(cs);content.addView(chart);loadHistory(s,chart,cs); technicalDashboard(s); v42RiskDashboard();
  ...
  HorizontalScrollView hsv=new HorizontalScrollView(this);hsv.setHorizontalScrollBarEnabled(false);LinearLayout tr=new LinearLayout(this);String[]tt={"Decision","Overview","Technicals","Candles","Fundamentals","Structure","Wyckoff","AI","Conviction","AI Brief","News"};for(String z:tt){Button b=btn(z);b.setOnClickListener(v->showIntel(s,tabs,z));LinearLayout.LayoutParams bp=new LinearLayout.LayoutParams(dp(112),dp(44));bp.setMargins(0,0,dp(6),0);tr.addView(b,bp);}hsv.addView(tr);tabs.addView(hsv);showIntel(s,tabs,"Decision");content.addView(tabs);
 }
...
void showIntel(Stock s,LinearLayout box,String mode){
  ...
  else if(mode.equals("Conviction")){x.setText("Loading backend conviction score…");loadConviction(s,x);}
  else if(mode.equals("Decision")){x.setText("Running full integrated analysis…");loadDecision(s,x);}
  else if(mode.equals("News")){x.setText("Loading news verdict…");loadNewsVerdict(s,x);}
  else if(mode.equals("AI Brief")){x.setText("Loading backend AI brief…");loadAIBriefBackend(s,x);}
  else if(mode.equals("AI")){x.setText("Building evidence-first research brief…");loadAIBrief(s,x);}
 ...
 String getJson(String path)throws Exception{
   URL u=new URL(BACKEND+path);HttpURLConnection c=(HttpURLConnection)u.openConnection();
   c.setConnectTimeout(20000);c.setReadTimeout(20000);c.setRequestProperty("Accept","application/json");
   return read(c.getInputStream());
 }
 void loadDecision(Stock s,TextView out){pool.submit(()->{try{
   String json=getJson("/decision/"+URLEncoder.encode(s.s,"UTF-8"));
   if(json==null||json.trim().isEmpty()||json.trim().equals("null")){h.post(()->out.setText("DECISION\n\nBackend returned no data."));return;}
   JSONObject o=new JSONObject(json);
   if("not_found".equals(o.optString("status",""))){h.post(()->out.setText("DECISION\n\nBackend has no current quote for "+s.s+"."));return;}
   JSONObject dec=o.optJSONObject("decision"),sc=o.optJSONObject("scoring"),en=o.optJSONObject("engines");
   StringBuilder z=new StringBuilder("INTEGRATED DECISION\n\n");
   if(dec!=null){
     z.append("SIGNAL: ").append(dec.optString("signal","—")).append("\n");
     z.append("Confluence: ").append(dec.optInt("confluence",0)).append("/4");
     JSONArray dims=dec.optJSONArray("confluence_dims");
     if(dims!=null&&dims.length()>0){z.append("  (");for(int i=0;i<dims.length();i++){if(i>0)z.append(", ");z.append(dims.optString(i));}z.append(")");}
     z.append("\n\n");
   }
   if(sc!=null){
     z.append("Score: ").append(String.format(Locale.US,"%.1f",sc.optDouble("final_score",0)))
      .append("   Confidence: ").append(String.format(Locale.US,"%.0f%%",sc.optDouble("confidence",0))).append("\n");
     String dq=sc.optString("data_quality","");if(dq.length()>0)z.append("Data quality: ").append(dq).append("\n");
     JSONObject bd=sc.optJSONObject("breakdown");
     if(bd!=null){z.append("\nSection scores\n");
       String[] ks={"technical","fundamentals","sentiment","macro_news"};
       for(String k:ks) if(bd.has(k)) z.append("• ").append(k).append(": ").append(String.format(Locale.US,"%.1f",bd.optDouble(k,0))).append("\n");}
     z.append("\n");
   }
   if(dec!=null){
     JSONArray rs=dec.optJSONArray("reasons");
     if(rs!=null&&rs.length()>0){z.append("Why\n");for(int i=0;i<rs.length();i++)z.append("• ").append(rs.optString(i)).append("\n");z.append("\n");}
   }
   if(en!=null){
     JSONObject t=en.optJSONObject("technical");
     if(t!=null){
       z.append("Technical evidence\n");
       z.append("• ").append(t.optString("classification","—")).append("  (score ").append(String.format(Locale.US,"%.1f",t.optDouble("score",0))).append(")\n");
       if(!t.isNull("rsi"))z.append("• RSI ").append(String.format(Locale.US,"%.1f",t.optDouble("rsi",0))).append("\n");
       z.append("• Support ").append(fmt(t.optDouble("support",0))).append("   Resistance ").append(fmt(t.optDouble("resistance",0))).append("\n");
       z.append("• Stop ").append(fmt(t.optDouble("stop_loss",0))).append("   Target ").append(fmt(t.optDouble("target1",0)));
       if(!t.isNull("headroom_rr"))z.append("   R:R ").append(String.format(Locale.US,"%.2f",t.optDouble("headroom_rr",0)));
       z.append("\n");
       if(!t.isNull("cmf"))z.append("• Money flow (CMF) ").append(String.format(Locale.US,"%+.3f",t.optDouble("cmf",0)))
         .append(t.optDouble("cmf",0)>0?"  confirming":"  NOT confirming").append("\n");
       else z.append("• Money flow: unavailable (needs stored OHLC)\n");
       String am=t.optString("atr_method","");if(am.length()>0)z.append("• ATR method: ").append(am).append("\n");
       z.append("\n");
     }
     JSONObject rk=en.optJSONObject("risk");
     if(rk!=null){
       z.append("Risk: ").append(rk.optString("risk_level","—")).append("\n");
       JSONArray vt=rk.optJSONArray("vetoes");
       if(vt!=null&&vt.length()>0){z.append("Vetoes: ");for(int i=0;i<vt.length();i++){if(i>0)z.append(", ");z.append(vt.optString(i));}z.append("\n");}
       JSONObject ps=rk.optJSONObject("position_sizing");
       if(ps!=null)z.append("Sizing: ").append(ps.optInt("suggested_shares",0)).append(" shares  •  max loss PKR ")
         .append(String.format(Locale.US,"%,.0f",ps.optDouble("max_loss_if_stopped_pkr",0))).append("\n");
       z.append("\n");
     }
     JSONObject rg=en.optJSONObject("regime");
     if(rg!=null)z.append("Market regime: ").append(rg.optString("regime","—")).append("\n");
     JSONObject sh=en.optJSONObject("shariah");
     if(sh!=null)z.append("Shariah: ").append(sh.optString("status","—")).append("\n");
     JSONObject nw=en.optJSONObject("news");
     if(nw!=null)z.append("News: ").append(nw.optString("direction","—"))
       .append(" (").append(nw.optString("confidence","")).append(")\n");
   }
   JSONObject un=o.optJSONObject("unavailable");
   if(un!=null&&un.length()>0){z.append("\nUnavailable layers\n");Iterator<String>k=un.keys();
     while(k.hasNext()){String kk=k.next();z.append("• ").append(kk).append(": ").append(un.optString(kk)).append("\n");}}
   JSONObject ig=o.optJSONObject("integration");
   if(ig!=null)z.append("\nCoverage: ").append(ig.optString("coverage","")).append("\n").append(ig.optString("meaning",""));
   String r=z.toString();h.post(()->out.setText(r));
 }catch(Exception e){h.post(()->out.setText("INTEGRATED DECISION\n\nBackend unavailable. No decision is generated locally."));}});}
 void loadNewsVerdict(Stock s,TextView out){pool.submit(()->{try{
   String json=getJson("/news-verdict/"+URLEncoder.encode(s.s,"UTF-8"));
   JSONObject o=new JSONObject(json);
   if(!"ok".equals(o.optString("status",""))){
     String note=o.optString("note","No news verdict available.");
     h.post(()->out.setText("NEWS\n\n"+note));return;}
   JSONObject v=o.optJSONObject("verdict");
   StringBuilder z=new StringBuilder("NEWS VERDICT\n\n");
   z.append("Direction: ").append(v.optString("direction","—"))
    .append("   Score: ").append(String.format(Locale.US,"%.1f",v.optDouble("score",50))).append("\n");
   z.append("Materiality: ").append(v.optString("materiality","—"))
    .append("   Confidence: ").append(v.optString("confidence","—")).append("\n\n");
   z.append(bulletSection("Headlines",v.optJSONArray("headlines")));
   z.append(bulletSection("Sources",v.optJSONArray("sources")));
   String cav=v.optString("caveat","");if(cav.length()>0)z.append("\n").append(cav);
   String r=z.toString();h.post(()->out.setText(r));
 }catch(Exception e){h.post(()->out.setText("NEWS\n\nBackend unavailable. No headline or sentiment is generated locally."));}});}
 void loadConviction(Stock s,TextView out){pool.submit(()->{try{
   URL u=new URL(BACKEND+"/conviction/"+URLEncoder.encode(s.s,"UTF-8"));
   HttpURLConnection c=(HttpURLConnection)u.openConnection();c.setConnectTimeout(12000);c.setReadTimeout(12000);c.setRequestProperty("Accept","application/json");
   String json=read(c.getInputStream());
   if(json==null||json.trim().isEmpty()||json.trim().equals("null")){h.post(()->out.setText("CONVICTION SCORE\n\nBackend returned no data for this symbol right now."));return;}
   JSONObject o=new JSONObject(json);
   if("not_found".equals(o.optString("status",""))){h.post(()->out.setText("CONVICTION SCORE\n\nBackend has no current quote for "+s.s+"."));return;}
   double score=o.optDouble("score",Double.NaN);String label=o.optString("label","");String meaning=o.optString("meaning","");
   StringBuilder z=new StringBuilder("CONVICTION SCORE (BACKEND)\n\n");
   z.append(Double.isNaN(score)?"Score unavailable":String.format(Locale.US,"%.1f / 100",score)).append("\n").append(label).append("\n\n");
   JSONObject comp=o.optJSONObject("components");
   if(comp!=null){z.append("Components\n");Iterator<String>keys=comp.keys();while(keys.hasNext()){String k=keys.next();z.append("• ").append(k).append(": ").append(String.format(Locale.US,"%.1f",comp.optDouble(k,0))).append("\n");}z.append("\n");}
   z.append(bulletSection("Reasons",o.optJSONArray("reasons")));
   JSONObject ds=o.optJSONObject("data_status");
   if(ds!=null)z.append("OHLC sessions stored: ").append(ds.optInt("ohlc_sessions",0)).append("\n");
   z.append("Eligible (≥50K vol): ").append(o.optBoolean("eligible",false)?"YES":"NO").append("\n\n");
   z.append(meaning);
   String rtxt=z.toString();h.post(()->out.setText(rtxt));
 }catch(Exception e){h.post(()->out.setText("CONVICTION SCORE\n\nBackend unavailable right now. This score is computed server-side and is not fabricated locally."));}});}
 void loadAIBriefBackend(Stock s,TextView out){pool.submit(()->{try{
   URL u=new URL(BACKEND+"/ai-brief/"+URLEncoder.encode(s.s,"UTF-8"));
   HttpURLConnection c=(HttpURLConnection)u.openConnection();c.setConnectTimeout(15000);c.setReadTimeout(15000);c.setRequestProperty("Accept","application/json");
   String json=read(c.getInputStream());
   if(json==null||json.trim().isEmpty()||json.trim().equals("null")){h.post(()->out.setText("AI BRIEF (BACKEND)\n\nBackend returned no data for this symbol right now — the /ai-brief endpoint has a known server-side issue where it only replies on its error path. Ask your backend to fix the indentation of the final return in ai_brief()."));return;}
   JSONObject o=new JSONObject(json);
   if("not_found".equals(o.optString("status",""))){h.post(()->out.setText("AI BRIEF\n\nBackend has no current quote for "+s.s+"."));return;}
   double score=o.optDouble("evidence_score",Double.NaN);String label=o.optString("label","");
   StringBuilder z=new StringBuilder("AI RESEARCH BRIEF (BACKEND)\n\n");
   if(!Double.isNaN(score))z.append(String.format(Locale.US,"Evidence score: %.1f / 100\n",score));
   z.append(label).append("\n\n");
   z.append(bulletSection("Bull case",o.optJSONArray("bull_case")));
   z.append(bulletSection("Bear / conflict case",o.optJSONArray("bear_case")));
   z.append(bulletSection("Confirmation",o.optJSONArray("confirmation")));
   z.append(bulletSection("Invalidation",o.optJSONArray("invalidation")));
   JSONObject news=o.optJSONObject("news_intelligence");
   if(news!=null){String st=news.optString("status","");if(st.length()>0)z.append("News intelligence: ").append(st).append("\n\n");}
   z.append(o.optString("llm_policy",""));
   String rtxt=z.toString();h.post(()->out.setText(rtxt));
 }catch(Exception e){h.post(()->out.setText("AI RESEARCH BRIEF\n\nBackend unavailable right now. No brief will be fabricated locally."));}});}
 String bulletSection(String title,JSONArray arr){if(arr==null||arr.length()==0)return "";StringBuilder b=new StringBuilder(title).append("\n");for(int i=0;i<arr.length();i++)b.append("• ").append(arr.optString(i)).append("\n");b.append("\n");return b.toString();}
 ...
 double macdSignal(ArrayList<Bar>b){double v=ema(b,9);return Double.isNaN(v)?Double.NaN:v;}
 ...
 String technicalEngine(ArrayList<Bar> b){
   ...
   String trend=fast>slow?"Bullish SMA bias (12>26)":"Bearish SMA bias (12<26)";
 ...
 // Real MACD: EMA12 - EMA26 with a 9-period signal line. Renders only what it
 // actually computes; when history is too short it says so rather than showing
 // a number derived from a different indicator.
 void macdEngine(ArrayList<Bar> b){
   LinearLayout c=card();
   c.addView(bold("MACD (EMA12 − EMA26)",17,TEXT));
   if(b==null||b.size()<35){
     c.addView(tv("Needs at least 35 sessions of history. No value shown.",12,MUTED));
   } else {
     double e12=ema(b,12), e26=ema(b,26), macd=e12-e26, sig=macdSignal(b);
     StringBuilder t=new StringBuilder();
     t.append("EMA12: ").append(fmt(e12)).append("   EMA26: ").append(fmt(e26)).append("\n");
     t.append("MACD line: ").append(String.format(Locale.US,"%+.2f",macd)).append("\n");
     if(!Double.isNaN(sig)){
       t.append("Signal (EMA9): ").append(fmt(sig)).append("\n");
       t.append("Histogram: ").append(String.format(Locale.US,"%+.2f",macd-sig)).append("\n");
     }
     t.append("Bias: ").append(macd>=0?"Positive (fast above slow)":"Negative (fast below slow)");
     c.addView(tv(t.toString(),12,MUTED));
   }
   content.addView(c);
 }
 // Real EMA trend stack. Each line is printed ONLY when enough history exists to
 // compute that EMA — a 200-EMA needs 200 sessions and is not approximated.
 void emaTrendSystem(ArrayList<Bar> b){
   LinearLayout c=card();
   c.addView(bold("EMA TREND STACK",17,TEXT));
   if(b==null||b.size()<20){c.addView(tv("Needs at least 20 sessions. No trend shown.",12,MUTED));content.addView(c);return;}
   double last=b.get(b.size()-1).c, e20=ema(b,20), e50=ema(b,50), e200=ema(b,200);
   StringBuilder t=new StringBuilder("Close: ").append(fmt(last)).append("\n");
   t.append("EMA20:  ").append(Double.isNaN(e20)?"need 20 sessions":fmt(e20)).append("\n");
   t.append("EMA50:  ").append(Double.isNaN(e50)?"need 50 sessions":fmt(e50)).append("\n");
   t.append("EMA200: ").append(Double.isNaN(e200)?"need 200 sessions":fmt(e200)).append("\n\n");
   int up=0,n=0;
   if(!Double.isNaN(e20)){n++;if(last>e20)up++;}
   if(!Double.isNaN(e50)){n++;if(last>e50)up++;}
   if(!Double.isNaN(e200)){n++;if(last>e200)up++;}
   t.append("Price above ").append(up).append(" of ").append(n).append(" available EMAs — ")
    .append(n==0?"no trend read":up==n?"aligned uptrend":up==0?"aligned downtrend":"mixed / transitional");
   c.addView(tv(t.toString(),12,MUTED));
   content.addView(c);
 }
 void v43PortfolioModule(){
   LinearLayout c=card();
   c.addView(bold("PORTFOLIO & SIGNAL HISTORY",17,TEXT));
   c.addView(tv("Not yet implemented. The local database schema exists (positions, signals, watchlist) but no trades are recorded and no P&L is computed from it yet.\n\nThis card is a placeholder and is labelled as one.",12,MUTED));
   content.addView(c);
 }
