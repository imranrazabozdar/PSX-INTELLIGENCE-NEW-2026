...
<style>
:root {
    --teal: #0D9488; --teal-dark: #0B7A70; --teal-light: #ECFDF9;
    --ink: #0F172A; --sub: #64748B; --line: #E6ECEA;
    --up: #0D9F6E; --up-bg: #E7F8F1; --down: #DC2626; --down-bg: #FDEDED;
}
#MainMenu, footer, header {visibility: hidden;}
.block-container {padding-top: 1.4rem; padding-bottom: 2rem; max-width: 1240px;}
html, body, [class*="css"] {font-family: -apple-system, "Segoe UI", Inter, Roboto, sans-serif;}
...
.psx-brand .mark {
    ...
    background: linear-gradient(135deg, var(--teal) 0%, #16BFAE 100%);
    display: flex; align-items: center; justify-content: center;
    font-size: 20px; box-shadow: 0 4px 10px rgba(13,148,136,0.25);
}
...
.psx-idx-pill {
    flex: 0 0 auto; min-width: 148px; background: #fff; border: 1px solid var(--line);
    border-radius: 14px; padding: 12px 16px; box-shadow: 0 1px 2px rgba(15,23,42,0.03);
}
...
.psx-card {
    ...
    padding: 16px 18px; box-shadow: 0 1px 3px rgba(15,23,42,0.04); margin-bottom: 10px;
}
...
.psx-avatar {
    ...
    display: flex; align-items: center; justify-content: center;
    font-size: 0.72rem; font-weight: 800; color: #fff; background: var(--teal);
}
...
.psx-chip {
    ...
    background: #fff; min-width: 150px; flex: 1 1 150px;
}
...
div[data-testid="stMetric"] {
    ...
    padding: 14px 16px 10px; box-shadow: 0 1px 3px rgba(20,35,30,0.04);
}
div[data-testid="stMetricLabel"] {font-weight: 600; color: #3B534B;}

.stTabs [data-baseweb="tab-list"] {gap: 4px; border-bottom: 1px solid var(--line);}
.stTabs [data-baseweb="tab"] {
    height: 42px; border-radius: 10px 10px 0 0; padding: 0 18px;
    font-weight: 600; color: #5C7268;
}
.stTabs [aria-selected="true"] {color: var(--teal-dark) !important; background: var(--teal-light);}
...
.psx-badge.watch {background: #FEF3C7; color: #B45309;}
.psx-badge.neutral {background: #F1F5F9; color: var(--sub);}
.psx-badge.pending {background: #EEF2F7; color: #94A3B8; font-style: italic;}
...
.psx-badge.grade-mid {background: #FEF3C7; color: #B45309; font-size: 1rem; padding: 6px 16px;}
...
.psx-conf-row .bar-track {flex: 0 0 90px; height: 8px; border-radius: 4px; background: #EEF2F7; position: relative; overflow: hidden;}
...
.psx-scenario-card.base {border-left-color: #94A3B8;}
</style>
...
_SYNC_GROUPS = {
    ...
    "admin_token": ["scan_token", "al_token", "bf_token", "bt_token", "dssscan_token", "wf_token",
                    "rg_token", "ed_token"],
}
_SYNC_DEFAULTS = {"home_min_vol": 50_000, "op_vol": 50_000, "al_vol": 50_000,
                  "op_limit": 50, "al_limit": 50, "bf_limit": 50,
                  "scan_token": "", "al_token": "", "bf_token": "", "bt_token": "", "dssscan_token": "",
                  "wf_token": "", "rg_token": "", "ed_token": ""}
...
tab_home, tab_screener, tab_pulse, tab_intel, tab_dss, tab_more = st.tabs(
    ["🏠 Home", "🔍 Screener", "📰 Pulse", "🧠 Intelligence", "🎯 Decision Support", "⚙️ More"]
)
...
with tab_home:
    ...
    with st.container(border=True):
        ...
        if mcc.get("status") == "ok":
            ...
            cc1.metric("KSE-100 Regime", mr.get("label", "—"))
            cc2.metric("Breadth", f"{mr.get('breadth_pct', '—')}%")
            cc3.metric("Momentum (avg chg)", f"{mr.get('avg_change_pct', '—')}%")
            cc4.metric("KSE-100 Volatility", vol.get("regime", "—") if vol.get("status") == "ok" else "—")
...
with tab_screener:
    st.markdown('<div class="psx-section-eyebrow">SCREENER</div>'
                '<div class="psx-section-title">Ranked Opportunities</div>', unsafe_allow_html=True)
    with st.container(border=True):
        c1, c2 = st.columns(2)
        with c1:
            op_min_vol = st.number_input("Min volume (opportunities)", step=10_000, key="op_vol",
                                          on_change=_sync_cb("min_volume", "op_vol"))
        with c2:
            op_limit = st.number_input("Limit", step=10, key="op_limit",
                                        on_change=_sync_cb("limit", "op_limit"))
        opp = _get("/opportunities", min_volume=op_min_vol, limit=op_limit)
        if isinstance(opp, list) and opp:
            df = pd.DataFrame(opp)
            df = df[[c for c in df.columns if not c.startswith("shariah")]]
    ...
    st.markdown('<div class="psx-section-eyebrow">DEEP SCAN</div>'
                '<div class="psx-section-title">Whole-Book Scan</div>', unsafe_allow_html=True)
    with st.container(border=True):
        st.caption("psx_brain verdicts over the most liquid names, ranked. Expensive — fetches "
                   "OHLCV + runs the full indicator engine per symbol. Requires PSX_ADMIN_TOKEN "
                   "on the backend unless called from localhost.")
        ...
        if st.button("Run scan", type="primary"):
            ...
            if r.get("status") == "forbidden":
                ...
                st.dataframe(_with_company(pd.DataFrame(r["ranked"])),
    ...
    st.markdown('<div class="psx-section-eyebrow">MARKET-WIDE</div>'
    ...
    with st.container(border=True):
        st.caption("Runs the full Decision Support System (confluence matrix, evidence score, Wyckoff, "
                   ...
                   "into named buckets. Same engine each stock's own DSS page uses — not a separate, "
                   "looser pass. The backend auto-refreshes this in the background, so loading this tab "
                   "is instant — prices are patched live on top of the cached analysis.")
        ...
        if status.get("status") == "ok":
            ...
            st.info("No scan cached yet — the backend runs one automatically shortly after startup "
                    "(can take 15-20+ minutes for the full backfilled universe), or force one now below.")
        ...
        # universe scan takes 15-20+ minutes, so this never blocks the page on
        # it. force just kicks off a background recompute; the response you
        ...
        if sr and "top_10_strongest_buy_setups" in sr:
            ...
            for label, key in buckets:
                ...
                st.dataframe(df_[[c for c in cols_per_row if c in df_.columns]],
...
# --------------------------------------------------------- Intelligence ----
with tab_intel:
    st.markdown('<div class="psx-section-eyebrow">MARKET-WIDE RANKING</div>'
                '<div class="psx-section-title">Top Ranked Opportunities — Whole PSX Market</div>',
                unsafe_allow_html=True)
    with st.container(border=True):
        st.caption("Conviction score (liquidity/momentum/trend/structure/Wyckoff/fundamentals blend) "
                   "over every liquid symbol in the live market — not a 48-symbol subset. Click a row "
                   "to load it below, or use the quick-pick / search underneath.")
        ...
        if isinstance(ranked, list) and ranked:
            ...
            sel = st.dataframe(rdf[display_cols], use_container_width=True, hide_index=True, height=380,
                               on_select="rerun", selection_mode="single-row", key="intel_rank_table")
            rows_sel = sel.selection.rows if sel and sel.selection else []
            if rows_sel:
                st.session_state.intel_symbol = rdf.iloc[rows_sel[0]]["symbol"]
        else:
            st.info("Ranked opportunities unavailable.")

    st.markdown('<div class="psx-section-eyebrow">STOCK RESEARCH</div>'
                '<div class="psx-section-title">Per-Stock Intelligence</div>', unsafe_allow_html=True)

    if "intel_symbol" not in st.session_state:
        st.session_state.intel_symbol = "OGDC"

    picks = ["OGDC", "PPL", "HBL", "UBL", "LUCK", "FFC", "ENGRO", "MCB", "SYS", "MARI"]
    pick_cols = st.columns(len(picks) + 2)
    for i, p in enumerate(picks):
        if pick_cols[i].button(p, key=f"pick_{p}",
                                type="primary" if st.session_state.intel_symbol == p else "secondary"):
            st.session_state.intel_symbol = p
    typed = pick_cols[-2].text_input("Or type a symbol", value="", key="intel_typed",
                                      label_visibility="collapsed", placeholder="Type symbol...")
    if typed.strip():
        st.session_state.intel_symbol = typed.strip().upper()

    symbol = st.session_state.intel_symbol
    if symbol:
        names = _company_names()
        _cname = names.get(symbol)
        _mrow = next((r for r in (_get("/market", min_volume=0) or [])
                      if isinstance(r, dict) and r.get("symbol") == symbol), None)
        _price = _mrow.get("price") if _mrow else None
        _pct = _mrow.get("pct") if _mrow else None
        _sector = _mrow.get("sector") if _mrow else None
        _cls = "up" if (_pct or 0) >= 0 else "down"
        _arrow = "▲" if (_pct or 0) >= 0 else "▼"
        st.markdown(
            f'<div class="psx-stock-hero"><div class="psx-avatar">{_initials(symbol)}</div>'
            f'<div><div class="sym">{symbol}</div>'
            f'<div class="co">{_cname or "—"}{" · " + _sector if _sector else ""}</div></div>'
            + (f'<div class="right"><div class="price">{_price}</div>'
               f'<div class="chg {_cls}">{_arrow} {abs(_pct):.2f}%</div></div>' if _price is not None else '')
            + '</div>', unsafe_allow_html=True)
        ...
        with colA:
            ...
            with st.container(border=True):
                st.markdown('<div class="psx-panel-title">⚖️ Decision (technical + fundamentals + news + risk)</div>',
                            unsafe_allow_html=True)
                d = _get(f"/decision/{symbol}", record=False)
        ...
        with colB:
            with st.container(border=True):
                st.markdown('<div class="psx-panel-title">📐 Wyckoff (true OHLC)</div>', unsafe_allow_html=True)
                w = _get(f"/wyckoff-pro/{symbol}")
                if w.get("phase"):
                    st.markdown(_badge(w["phase"]), unsafe_allow_html=True)
                    tr = w.get("trading_range", {})
                    if tr:
                        st.caption(f"Range: {tr.get('support')} – {tr.get('resistance')} "
                                   f"({tr.get('width_pct')}% wide, {tr.get('bars')} bars)")
            ...
            with st.container(border=True):
                ...
                t = _get(f"/technical-pro/{symbol}")
                if t.get("status") == "ok":
                    tech = t["technical"]
                    st.markdown(_badge(tech.get("classification")) +
                                f'&nbsp;&nbsp;score **{tech.get("score")}**', unsafe_allow_html=True)
                    m1, m2, m3 = st.columns(3)
                    m1.metric("RSI", tech.get("rsi") and round(tech["rsi"], 1))
                    m2.metric("ATR%", tech.get("atr_pct") and round(tech["atr_pct"], 2))
                    m3.metric("ADX", tech.get("adx_proxy") and round(tech["adx_proxy"], 1))
                    if tech.get("notes"):
            ...
            with st.container(border=True):
                st.markdown('<div class="psx-panel-title">🕯️ Candlestick patterns & structure (true OHLC)</div>',
                            unsafe_allow_html=True)
                it = _get(f"/intelligence/{symbol}")
        ...
        st.markdown('<div class="psx-section-eyebrow">FUNDAMENTALS</div>'
                     '<div class="psx-section-title">Fundamentals — free (PSX company page + sector blend)</div>',
                     unsafe_allow_html=True)
        fc1, fc2 = st.columns(2)
        with fc1, st.container(border=True):
            fund_engine = (d.get("engines", {}) or {}).get("fundamentals") if isinstance(d, dict) else None
        ...
        with fc2, st.container(border=True):
            fp = _get(f"/fundamentals/{symbol}")
            if fp.get("raw_available"):
                st.table(pd.DataFrame(
                    [(k, v) for k, v in fp.items() if k not in ("symbol", "source", "raw_available")],
                    columns=["field", "value"]))
                st.caption(f"Scraped live from {fp.get('source')}")
            else:
                st.info("PSX company-page scrape unavailable for this symbol.")

        with st.expander("psxwatch deep engine (53-metric ratios, forensics-adjacent) — needs ANTHROPIC_API_KEY"):
            st.caption("This is a separate, deeper engine (DIO/DSO/CCC, margins, leverage, "
                       "forensic screens) that transcribes real filing PDFs with an LLM — "
                       "the free scrape above is not a substitute for it, just what's "
                       "available without an API key.")
            fd = _get(f"/fundamentals-deep/{symbol}")
            if fd.get("status") == "ok":
                ...
                st.dataframe(
                    pd.DataFrame(sorted(ratios.items()), columns=["metric", "value"]),
        ...
        with c1, st.container(border=True):
            st.markdown('<div class="psx-panel-title">📰 News — free (deterministic keyword scorer)</div>',
                        unsafe_allow_html=True)
            nv = _get(f"/news-verdict/{symbol}")
            if nv.get("status") == "ok":
                v = nv["verdict"]
                st.markdown(_badge(v.get("direction")) +
                            f'&nbsp;&nbsp;materiality **{v.get("materiality")}** · '
                            f'confidence **{v.get("confidence")}**', unsafe_allow_html=True)
                if v.get("headlines"):
                    st.write("\n".join("• " + h for h in v["headlines"]))
                if v.get("sources"):
                    st.caption(" · ".join(v["sources"][:3]))
            else:
                st.info(nv.get("note", "No fresh headline matched this ticker this run."))
            with st.expander("psxwatch evidence-quoted scores — needs ANTHROPIC_API_KEY"):
                ni = _get(f"/news-impact/{symbol}")
                if ni.get("status") == "ok" and ni.get("items"):
                    st.dataframe(pd.DataFrame(ni["items"]), use_container_width=True)
                else:
                    st.info(ni.get("reason", "No stored news impact scores."))
        with c2, st.container(border=True):
            st.markdown('<div class="psx-panel-title">🎯 Driver exposure (psxwatch priors)</div>',
                        unsafe_allow_html=True)
            ex = _get(f"/exposure/{symbol}")
...
# ------------------------------------------------------ Decision Support ----
with tab_dss:
    st.markdown('<div class="psx-section-eyebrow">DECISION SUPPORT SYSTEM</div>'
                '<div class="psx-section-title">Quantitative Technical Intelligence</div>',
                unsafe_allow_html=True)
    st.caption("Hierarchy of evidence: Market Regime → Sector Regime → Stock Structure → "
               "Wyckoff → Supply/Demand → Candlesticks → Price Action → Relative Strength → "
               "Risk/Reward → Final Decision. Historical pattern statistics and quant baseline "
               "validation are explicitly marked PENDING below — no win-rate is fabricated. "
               "This is analysis, not certainty: read every action as a hypothesis, not a promise.")

    st.markdown('<div class="psx-section-eyebrow">MARKET-WIDE RANKING</div>'
                '<div class="psx-section-title">Decision Support Scan — Every Backfilled Symbol</div>',
                unsafe_allow_html=True)
    with st.container(border=True):
        dscan = _get("/dss-scan")
        if "top_10_strongest_buy_setups" in dscan:
            age_s = dscan.get("_cache_age_seconds")
            st.caption(f"{dscan.get('scanned')} symbols scanned"
                       + (f" · cached {round(age_s/60,1)} min ago" if age_s is not None else "")
                       + " · click a row to load it below.")
            bucket_keys = [k for k in dscan if k.startswith("top_10_")]
            merged = {}
            for k in bucket_keys:
                for item in dscan.get(k, []):
                    sym = item.get("symbol")
                    if sym not in merged or (item.get("score") or 0) > (merged[sym].get("score") or 0):
                        merged[sym] = item
            if merged:
                mdf = pd.DataFrame(list(merged.values())).sort_values("score", ascending=False)
                cols_show = [c for c in ["symbol", "name", "sector", "price", "pct", "phase",
                                         "signal_stage", "score", "grade", "action", "rs_vs_index"]
                            if c in mdf.columns]
                dsel = st.dataframe(mdf[cols_show], use_container_width=True, hide_index=True, height=380,
                                    on_select="rerun", selection_mode="single-row", key="dss_rank_table")
                drows = dsel.selection.rows if dsel and dsel.selection else []
                if drows:
                    st.session_state.dss_symbol = mdf.iloc[drows[0]]["symbol"]
            else:
                st.info("Scan ran but nothing landed in a bucket today — see the Screener tab for the "
                        "full 8-category breakdown.")
        else:
            st.info(dscan.get("reason", "Market scan not cached yet — the background scheduler runs one "
                                        "automatically shortly after startup. See Screener tab for status."))

    if "dss_symbol" not in st.session_state:
        st.session_state.dss_symbol = st.session_state.get("intel_symbol", "OGDC")
    ...
    for i, p in enumerate(dss_picks):
        if dss_cols[i].button(p, key=f"dsspick_{p}",
                               type="primary" if st.session_state.dss_symbol == p else "secondary"):
            st.session_state.dss_symbol = p
    dss_typed = dss_cols[-2].text_input("Or type a symbol", value="", key="dss_typed",
                                         label_visibility="collapsed", placeholder="Type symbol...")
    if dss_typed.strip():
        st.session_state.dss_symbol = dss_typed.strip().upper()
    dss_symbol = st.session_state.dss_symbol
    ...
    if dss_symbol:
        ...
        if d.get("status") != "ok":
            ...
            wev = _get(f"/wyckoff-events/{dss_symbol}")
...
with tab_more:
    ...
    with st.container(border=True):
        ...
        with bf2:
            ...
            bf_token = _admin_token_input("bf_token")
            if st.button("Backfill bulk"):
                params = {"limit": bf_limit}
                if bf_token:
                    params["token"] = bf_token
    ...
    with st.container(border=True):
        ...
        bt_token = _admin_token_input("bt_token")
        if st.button("Force re-run now", type="secondary"):
            params = {"force": "true"}
            if bt_token:
                params["token"] = bt_token
    ...
    with st.container(border=True):
        ...
        wf_token = _admin_token_input("wf_token")
        if st.button("Force re-run walk-forward now", type="secondary"):
            params = {"force": "true"}
            if wf_token:
                params["token"] = wf_token
    ...
    with st.container(border=True):
        ...
        rg_token = _admin_token_input("rg_token")
        if st.button("Force re-run regime-split now", type="secondary"):
            params = {"force": "true"}
            if rg_token:
                params["token"] = rg_token
    ...
    with st.container(border=True):
        ...
        st.caption("Tests combinations beyond the fixed textbook pattern library — RVOL>90th-percentile "
                   ...
                   "Auto-refreshes daily — no click needed for routine use.")
        ed_token = _admin_token_input("ed_token")
